...
const util = {};
util.setTitle = title => process.stdout.write(`\u001b]1337;SetUserVar=name=${Buffer.from(title).toString('base64')}\u0007`);
util.resolvePaths = (paths) => find(paths, p => fs.existsSync(p));
util.resolveFileInPaths = (paths, file) => util.resolvePaths(map(paths, p => path.resolve(p, file)));
...
exports.lcList = (prefix) => sortBy(map(filter(flatMap(plistDir, plDir => fs.readdirSync(plDir)), i => startsWith(i, prefix)), i => i.replace(new RegExp(`^${escapeRegExp(prefix)}(.*)\.plist$`, 'g'), '$1')));
